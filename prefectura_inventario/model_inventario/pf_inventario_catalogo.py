...
class Catalogo(models.Model):
    _name = 'pf_inventario.catalogo'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'Catálogo'

    name = fields.Char(string="Nombre del Catalogo", required=True, tracking=True)
    items_ids = fields.One2many(string='Catalogo', comodel_name='pf_inventario.items', inverse_name='catalogo_id',)
...
class Items(models.Model):
    ...
    catalogo_id = fields.Many2one(string='Catalogo', comodel_name='pf_inventario.catalogo', ondelete='restrict',required=True, tracking=True)   
